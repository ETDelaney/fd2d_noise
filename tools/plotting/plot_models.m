...
function plot_models(structure, source, array, platform, clim)
...
%     array = array / 1;
    ...
    if (~isempty(structure))
        ...
        %- plot platform if given --------------------------------------------
        if (~isempty(platform))
            handle(end + 1,:) = plot(ax1, platform(1, 1), platform(1, 2), ... % level(1) + 0 * array(:, 2), ...
                'kp', 'MarkerFaceColor', 'k', 'MarkerSize', 6);
            legend_string{end + 1} = 'array';
        end


        %- include legend -------------------------------------------------
        legend(ax1, handle, legend_string, 'position', [0.45, 0.02, 0.1, 0.05])
        ...
        cb = colorbar;
        ylabel(cb, '(m/s)')
        m = max(max(abs(structure - 360)));
        caxis(ax1, [360 - m - double(m < 1) * 50, 360 + m + double(m < 1) * 50])
        ...
        set(ax1, 'XTick', [0, round(Lx/2), Lx])
        set(ax1, 'YTick', [0, round(Lz/2), Lz])
        xlim(ax1, [0, Lx])
        ylim(ax1, [0, Lz])
        xlabel(ax1, 'x (m)')
        ylabel(ax1, 'y (m)')
    ...
    if (~isempty(source))
        ...
        %- plot platform if given --------------------------------------------
        if (~isempty(platform))
            handle(end + 1,:) = plot(ax2, platform(1, 1), platform(1, 2), ... % level(1) + 0 * array(:, 2), ...
                'kp', 'MarkerFaceColor', 'k', 'MarkerSize', 6);
            legend_string{end + 1} = 'array';
        end
